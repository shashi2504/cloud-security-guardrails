...
**Status:** BLOCK/IGNORE rows are the uncontested baseline. The 16 DECIDE rows
cover four judgment calls (egress breadth, IAM check redundancy, CMK cost,
access logging) and are unresolved.
...
| tfsec | aws-ec2-no-public-egress-sgr | CRITICAL | An egress security group rule allows traffic to /0. | DECIDE | CRITICAL per tfsec vs AWS default. Blocks exfil/C2 but may make gate unusable. |
| tfsec | aws-ec2-no-public-ingress-sgr | CRITICAL | An ingress security group rule allows traffic from /0. | BLOCK |  |
| tfsec | aws-ec2-volume-encryption-customer-key | LOW | EBS volume encryption should use Customer Managed Keys | DECIDE | CMK at $1/key/mo — security requirement or cost decision? |
...
| tfsec | aws-s3-enable-bucket-logging | MEDIUM | S3 Bucket does not have logging enabled. | DECIDE | Only way to know a public bucket was read. Exposure-adjacent? |
| tfsec | aws-s3-enable-versioning | MEDIUM | S3 Data should be versioned | WARN |  |
| tfsec | aws-s3-encryption-customer-key | HIGH | S3 encryption should use Customer Managed Keys | DECIDE | CMK at $1/key/mo — security requirement or cost decision? |
...
| checkov | CKV_AWS_382 |  | Ensure no security groups allow egress from 0.0.0.0:0 to port -1 | DECIDE | Same call as tfsec egress rule — must match. |
...
| checkov | CKV_AWS_189 |  | Ensure EBS Volume is encrypted by KMS using a customer managed Key (CMK) | DECIDE | CMK cost call — must match tfsec CMK rules. |
| checkov | CKV_AWS_290 |  | Ensure IAM policies does not allow write access without constraints | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_287 |  | Ensure IAM policies does not allow credentials exposure | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_288 |  | Ensure IAM policies does not allow data exfiltration | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_355 |  | Ensure no IAM policies documents allow "*" as a statement's resource for restrictable actions | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_62 |  | Ensure IAM policies that allow full "*-*" administrative privileges are not created | BLOCK |  |
| checkov | CKV_AWS_289 |  | Ensure IAM policies does not allow permissions management / resource exposure without constraints | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_63 |  | Ensure no IAM policies documents allow "*" as a statement's actions | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_286 |  | Ensure IAM policies does not allow privilege escalation | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
...
| checkov | CKV_AWS_18 |  | Ensure the S3 bucket has access logging enabled | DECIDE | Same call as tfsec logging rule — must match. |
...
| checkov | CKV2_AWS_40 |  | Ensure AWS IAM policy does not allow full IAM privileges | DECIDE | One of 8 IAM checks on one wildcard. All BLOCK, or 1 BLOCK + 7 WARN? |
| checkov | CKV_AWS_145 |  | Ensure that S3 buckets are encrypted with KMS by default | DECIDE | CMK cost call — must match tfsec CMK rules. |
